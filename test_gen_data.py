...
if __name__ == '__main__':
    ...
    for i, data in enumerate(dataset):
        ...
        pred = visuals['fake_B'][0].detach().cpu().numpy()
        pred = (np.transpose(pred, (1, 2, 0)) + 1) / 2.0 * 255.0 
        pred = pred.astype(np.float32)
        pred_path = img_path[0].split('/')[-1].replace('.png', '')
        pred_path = f'{pred_path}.np'
        ...
        scores_dict[img_path[0].split('/')[-1]] = [PSNR(visuals['real_B'], visuals['fake_B'], reduction='mean').detach().cpu().numpy().item(), SSIM(visuals['real_B'], visuals['fake_B'], reduction='mean').detach().cpu().numpy().item()]
